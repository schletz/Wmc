...
namespace WmcApi.HolidayCalendar;
...
public class CalendarService
{
    ...
    /// <summary>
    /// Ermittelt das Datum, wann die übergebene Anzahl an Schultagen vergangen sind.
    /// </summary>
    public DateTime AddSchoolDays(DateTime start, int schoolDays)
    {
        var startDay = _calendarDays[CalcDayNumber(start)];
        var counter = startDay.SchooldayCounter + schoolDays;
        // Bei negativen Werten wird der Tag davor gesucht, da der Counter an freien Tagen
        // seinen Wert beibehät (also 34, 35, 35, 35, 36).
        if (schoolDays < 0) counter--;
        int i = CalcDayNumber(start.AddDays(schoolDays));
        while (_calendarDays[i].SchooldayCounter != counter)
        {
            i += schoolDays < 0 ? -1 : 1;
            if (i < 0 || i >= _calendarDays.Length) { throw _outOfRangeException; }
        }
        if (schoolDays < 0) i++; // Der erste Tag der Serie mit gleichen Counter Werten.
        return _calendarDays[i].DateTime;
    }
    ...
    /// </summary>
    public int CalcDayNumber(DateTime date)
    {
